function generateLevel(){
    ...
    game.monsters = [];
    generateMonsters(game.tiles.startTile);

    game.items = [];
    game.cells = 3+Math.floor(game.level/5);
    game.cellsCollected = 0;
    for(let i=0;i<game.cells;i++) {
        game.items.push(new Treasure(randomPassableTile()));
    }
...
function randomPassableTile(playerPos=null, standable=false, standable_not_passable=true){
    let tile;
    tryTo('get random passable tile', function(){
        let x = randomRange(0, game.tiles.dimW-1);
        let y = randomRange(0, game.tiles.dimH-1);
        tile = game.tiles.at([x, y]);
        if(playerPos!=null)
...
function generateMonsters(playerPos){
    let numMonsters = Math.min(2*game.level, 15) + 5;
    for(let i=0;i<numMonsters;i++){
        spawnMonster(playerPos, standable=false);
    }
}

function spawnMonster(playerPos=null, standable=false){
    let m = [];
    m = [Jelly];
    // if(game.level<=5)
    ...
    let monster = new monsterType(randomPassableTile(playerPos, standable));
